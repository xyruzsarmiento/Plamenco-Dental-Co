...
Deno.serve(async (request) => {
  ...
  if (branchIds.length) {
    const { data: validBranches, error: branchError } = await adminClient.from('branches').select('id').in('id', branchIds).eq('status', 'active')
  ...
  if (role === 'dentist' || role === 'associate_dentist') {
    const { data: provider, error: providerError } = await adminClient.from('providers').upsert({ profile_id: userId, display_name: name, role, email, status: 'active' }, { onConflict: 'profile_id' }).select('id').single()
    if (providerError || !provider) return json({ error: `Invitation sent, but dentist profile provisioning failed: ${providerError?.message ?? 'Provider row missing.'}`, invitation }, 500)
    if (branchIds.length) {
      const providerAssignments = branchIds.map((branchId, index) => ({ provider_id: provider.id, branch_id: branchId, is_primary: index === 0, status: 'active' }))
      const { error: assignmentError } = await adminClient.from('provider_branch_assignments').upsert(providerAssignments, { onConflict: 'provider_id,branch_id' })
      if (assignmentError) return json({ error: `Invitation sent, but dentist branch assignment failed: ${assignmentError.message}`, invitation }, 500)
    }
  } else if (branchIds.length) {
    const staffAssignments = branchIds.map((branchId, index) => ({ profile_id: userId, branch_id: branchId, is_primary: index === 0, status: 'active' }))
    const { error: assignmentError } = await adminClient.from('staff_branch_assignments').upsert(staffAssignments, { onConflict: 'profile_id,branch_id' })
    if (assignmentError) return json({ error: `Invitation sent, but branch assignment failed: ${assignmentError.message}`, invitation }, 500)
  }

  return json({ invitation, account: { userId, email, role, branchIds, providerProfileCreated: role === 'dentist' || role === 'associate_dentist' } })
})
