create or replace function public.provider_bookable_for_slot_v133(
  p_provider_id uuid,
  p_branch_id uuid,
...
declare
  v_day int;
begin
  if p_provider_id is null or p_branch_id is null or p_appointment_date is null or p_start_time is null or p_end_time is null then
    return false;
  end if;
  ...
  if not exists (
    ...
    join public.provider_branch_assignments pba on pba.provider_id = p.id
  ...
  if not exists (
    ...
    where b.id = p_branch_id
      and b.status = 'active'
      and p_start_time >= b.opening_time
      and p_end_time <= b.closing_time
  ) then
    return false;
  end if;
  ...
  if exists (
    ...
    v_day := extract(dow from p_appointment_date)::int;
    if not exists (
      select 1
      from public.provider_schedule_blocks psb
      where psb.provider_id = p_provider_id
        and psb.branch_id = p_branch_id
        and psb.status = 'active'
        and psb.day_of_week = v_day
        and p_start_time >= psb.start_time
        and p_end_time <= psb.end_time
    ) then
      return false;
    end if;
  end if;
...
create or replace function public.patient_booking_available_provider_count_v133(
  p_branch_id uuid,
  p_service_id uuid,
  p_appointment_date date,
  p_start_time time
...
begin
  ...
  if not found or coalesce(v_duration, 0) <= 0 then
    return 0;
  end if;

  v_end_time := (p_start_time + (v_duration * interval '1 minute'))::time;

  select count(*)
    into v_count
  from public.providers p
  where public.provider_bookable_for_slot_v133(p.id, p_branch_id, p_appointment_date, p_start_time, v_end_time);
...
begin
  ...
  if public.patient_booking_available_provider_count_v133(p_branch_id, p_service_id, p_appointment_date, v_start_time) <= 0 then
    raise exception 'No dentists are available at this time. Choose another time or date.';
  end if;
...
revoke all on function public.provider_bookable_for_slot_v133(uuid, uuid, date, time, time, uuid) from public, anon;
revoke all on function public.patient_booking_available_provider_count_v133(uuid, uuid, date, time) from public, anon;
