-- Allow a revoked Dentist/Associate Dentist to complete a fresh invitation
-- without creating a second auth user, provider, or branch assignment.
...
begin
  ...
  select p.role, p.status into v_profile_role, v_profile_status
  ...
  select i.id, i.role into v_invitation_id, v_invitation_role
  ...
  if v_invitation_role in ('dentist', 'associate_dentist') then
