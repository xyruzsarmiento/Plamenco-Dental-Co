...
function getRoleCopy(role?: UserRole) {
  if (role === 'super_admin') return { eyebrow: 'Executive dashboard', subtitle: "Here's the current workspace picture for leadership decisions today.", icon: ShieldCheck, signal: 'Owner view', actions: [{ label: 'Reports', path: '/app/reports', icon: BarChart3 }, { label: 'Settings', path: '/app/settings', icon: Settings }] }
  if (role === 'dentist' || role === 'associate_dentist') return { eyebrow: 'Clinical dashboard', subtitle: "Here's your patient flow and clinical schedule for the day.", icon: Stethoscope, signal: 'Clinical view', actions: [{ label: 'My schedule', path: '/app/appointments', icon: CalendarDays }, { label: 'Patient records', path: '/app/dental-records', icon: FileText }] }
  return { eyebrow: 'Front desk dashboard', subtitle: "Here's what's happening at the clinic today.", icon: UsersRound, signal: 'Staff view', actions: [{ label: 'Book visit', path: '/app/appointments', icon: CalendarDays }, { label: 'Add patient', path: '/app/patients', icon: UserPlus }] }
}

type DashboardGreetingProps = { variant?: 'internal' | 'patient'; eyebrow?: string; name?: string; subtitle?: string; signal?: string; icon?: ReactNode; actions?: ReactNode }
...
export function DashboardGreeting({ actions, eyebrow, icon, name, signal, subtitle, variant = 'internal' }: DashboardGreetingProps) {
  ...
  const workspaceSubtitle = user?.role === 'super_admin' && branchContext
    ? branchContext.isAllBranchesMode
      ? 'All Branches executive overview. Operational records remain separated by branch.'
      : branchContext.activeBranch
        ? `Showing ${branchContext.activeBranch.name} data in this dashboard workspace.`
        : roleCopy.subtitle
    : roleCopy.subtitle
  const workspaceSignal = user?.role === 'super_admin' && branchContext
    ? branchContext.isAllBranchesMode ? 'All Branches' : branchContext.activeBranch?.name.replace(/^Plamenco Dental Co\.\s*-\s*/i, '') || roleCopy.signal
    : roleCopy.signal
  const signalLabel = signal ?? workspaceSignal
