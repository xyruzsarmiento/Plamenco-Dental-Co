...
export function ExpensesHistoricalWorkspaceV129(){
  ...
  useEffect(()=>{if(user?.role!=='super_admin'||!supabase)return;let active=true;void supabase.rpc('get_management_report_v129',{p_start_date:start,p_end_date:end,p_branch_id:scope}).then(({data:payload,error:rpcError})=>{if(!active)return;if(rpcError)setError(rpcError.message);else{setError(null);setData(payload as History)}});return()=>{active=false}},[end,scope,start,user?.role])
