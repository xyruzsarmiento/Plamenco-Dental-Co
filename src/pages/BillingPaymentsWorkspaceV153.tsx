...
function patientRecord(patientId: string) {
  return getStoredPatients().find((entry) => entry.id === patientId || entry.patientId === patientId)
}
      ...
      <div className="bp153-invoice-layout">
        ...
        <aside className="bp153-invoice-summary" aria-label="Invoice summary">
          ...
          <div className="bp153-summary-facts">
            <div><span>Patient</span>{selectedPatient ? <span className="patient-identity-inline"><PatientAvatar patient={selectedPatient} size="small" /><strong>{`${selectedPatient.firstName} ${selectedPatient.lastName}`.trim()}</strong></span> : <strong>Not selected</strong>}</div>
...
export function BillingPaymentsWorkspaceV153() {
  ...
  const detailPatientId = detail?.kind === 'invoice' ? detail.invoice.patientId : detail?.kind === 'payment' ? detail.payment.patientId : detail?.receipt.patientId
  const detailPatient = detailPatientId ? patientRecord(detailPatientId) : undefined
  ...
  return <section className="bp153-page">
    ...
    <section className="bp153-workspace">
      ...
      {tab === 'invoices' || tab === 'receivables' ? <div className="bp153-table-wrap">
        ...
        <table className="bp153-table">
          ...
          <tbody>{(pagedRows as Payment[]).map((payment) => { const receipt = data.receipts.find((entry) => entry.paymentId === payment.id); const patient = patientRecord(payment.patientId); return <tr key={payment.id} tabIndex={0} onClick={() => setDetail({ kind: 'payment', payment })} onKeyDown={(event) => rowKeyboard(event, () => setDetail({ kind: 'payment', payment }))}><td data-label="Payment #">{payment.paymentNumber}</td><td data-label="Patient"><span className="patient-identity-inline">{patient && <PatientAvatar patient={patient} size="compact" decorative />}<span>{patientName(payment.patientId)}</span></span></td><td data-label="Method">{getPaymentMethodLabel(payment.paymentMethod)}</td><td data-label="Amount">{formatCurrency(payment.amountCents)}</td><td data-label="Date">{dateLabel(payment.date)}</td><td data-label="Status"><StatusBadge status={payment.status} variant="compact"/></td><td data-label="Receipt">{receipt?.receiptNumber ?? 'Pending'}</td></tr> })}</tbody>
        </table>
      </div> : <div className="bp153-receipts">
        {(pagedRows as Receipt[]).map((receipt) => { const patient = patientRecord(receipt.patientId); return <button key={receipt.id} type="button" onClick={() => setDetail({ kind: 'receipt', receipt })}>{patient ? <PatientAvatar patient={patient} size="small" decorative /> : <ReceiptText size={17}/>}<span><strong>{receipt.receiptNumber}</strong><small>{patientName(receipt.patientId)} - {dateLabel(receipt.issuedAt)}</small></span><b>{formatCurrency(receipt.amountCents)}</b></button> })}
      </div>}
    ...
    {detail && <Drawer title="Billing detail" onClose={() => setDetail(null)}>
      <header className="bp153-drawer-head"><div><span>{detail.kind}</span><h2>{detail.kind === 'invoice' ? detail.invoice.invoiceNumber : detail.kind === 'payment' ? detail.payment.paymentNumber : detail.receipt.receiptNumber}</h2><p className="patient-identity-inline">{detailPatient && <PatientAvatar patient={detailPatient} size="small" />}{patientName(detailPatientId ?? '')}</p></div><button type="button" onClick={() => setDetail(null)} aria-label="Close detail"><X size={18}/></button></header>
