...
import {
  ...
  postStockCountPersisted,
  receiveStockTransferPersisted,
} from '../features/inventory/inventoryPersistence'
import { reviewStockCountPersisted } from '../features/inventory/inventorySetupPersistence'
    ...
    <PageScaffold title="Inventory" description="Track clinic supplies, materials and branch stock levels.">
      <div className="page-stack">
        ...
        <div className="filter-panel" style={{ display: 'grid', gridTemplateColumns: 'minmax(220px, 1fr) repeat(3, minmax(150px, 220px))', gap: 12 }}>
          <label className="field"><span>Search inventory</span><div style={{ position: 'relative' }}><Search size={15} style={{ position: 'absolute', left: 12, top: 13, color: 'var(--text-muted)' }} /><input value={search} onChange={(event) => setSearch(event.target.value)} style={{ paddingLeft: 36 }} /></div></label>
          ...
          <div className="workspace-grid">
            <section className="workspace-panel">
              ...
              <div className="workspace-list">
                {filteredItems.map((item) => {
                  ...
                  return <button key={item.id} type="button" className="workspace-row" onClick={() => setSelectedItemId(item.id)}><div><strong>{item.name}</strong><span>{item.itemCode} - {item.sku || 'No SKU'} - {getInventoryCategories().find((category) => category.id === item.categoryId)?.name ?? item.categoryId}</span><small>{item.brand || 'No brand'} - {getInventoryUnits().find((unit) => unit.id === item.unitId)?.label ?? item.unitId}</small></div><div style={{ display: 'grid', gap: 6, minWidth: 210 }}>{branchStocks.map(({ branch, stock }) => <div key={branch.id} style={{ display: 'flex', justifyContent: 'space-between', gap: 10 }}><span>{branch.city || branch.name}</span><Badge tone={statusTone(getStockStatus(stock))}>{stock ? formatQuantity(stock.quantityOnHand, item.unitId) : '0'}</Badge></div>)}</div></button>
                })}
            ...
            <aside className="workspace-panel">
              {selectedItem ? <>
                ...
                <div className="detail-grid detail-grid-mini"><div className="detail-item"><span>Category</span><strong>{getInventoryCategories().find((category) => category.id === selectedItem.categoryId)?.name ?? selectedItem.categoryId}</strong></div><div className="detail-item"><span>Unit</span><strong>{getInventoryUnits().find((unit) => unit.id === selectedItem.unitId)?.label ?? selectedItem.unitId}</strong></div><div className="detail-item"><span>Batch tracking</span><strong>{selectedItem.trackBatches ? 'Enabled' : 'Off'}</strong></div><div className="detail-item"><span>Expiry tracking</span><strong>{selectedItem.trackExpiry ? `${selectedItem.expiryWarningDays} days` : 'Off'}</strong></div></div>
                <div className="workspace-list">{branches.map((branch) => { const stock = getBranchStock(branch.id, selectedItem.id); return <div key={branch.id} className="workspace-row"><div><strong>{branch.name}</strong><span>Reorder level {stock?.reorderLevel ?? selectedItem.defaultReorderLevel}</span></div><div><Badge tone={statusTone(getStockStatus(stock))}>{getStockStatus(stock).replaceAll('_', ' ')}</Badge><strong>{formatQuantity(stock?.quantityOnHand ?? 0, selectedItem.unitId)}</strong></div></div> })}</div>
                <div className="toolbar-row">{permissions.can('inventory.stock_in') && <Button size="sm" icon={<PackagePlus size={14} />} onClick={() => setDialog({ type: 'stock_in', item: selectedItem })}>Stock In</Button>}{permissions.can('inventory.stock_out') && <Button size="sm" variant="secondary" onClick={() => setDialog({ type: 'stock_out', item: selectedItem })}>Stock Out</Button>}{permissions.can('inventory.adjust') && <Button size="sm" variant="secondary" onClick={() => setDialog({ type: 'adjust', item: selectedItem })}>Adjust</Button>}{permissions.can('inventory.transfer') && <Button size="sm" variant="secondary" icon={<ArrowRightLeft size={14} />} onClick={() => setDialog({ type: 'create_transfer', item: selectedItem })}>Create Transfer</Button>}{permissions.can('inventory.transfer') && <Button size="sm" variant="secondary" onClick={() => setDialog({ type: 'quick_transfer', item: selectedItem })}>Quick Transfer</Button>}</div>
                <div className="workspace-list">{getInventoryBatches().filter((batch) => batch.itemId === selectedItem.id).slice(0, 5).map((batch) => <div key={batch.id} className="workspace-row"><div><strong>{batch.batchNumber}</strong><span>{batch.expiryDate || 'No expiry'} - {branchName(batch.branchId)}</span></div><Badge tone={statusTone(getExpiryStatus(batch, selectedItem))}>{getExpiryStatus(batch, selectedItem).replaceAll('_', ' ')}</Badge></div>)}</div>
                <div className="workspace-list">{getItemMovements(selectedItem.id).slice(0, 8).map((movement) => <div key={movement.id} className="workspace-row"><div><strong>{movement.movementType.replaceAll('_', ' ')}</strong><span>{branchName(movement.branchId)} - {movement.reason}</span><small>{movement.performedBy}</small></div><strong>{movement.quantityBefore} {'->'} {movement.quantityAfter}</strong></div>)}</div>
