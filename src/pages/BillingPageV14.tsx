...
function statusTone(status: string): 'success' | 'warning' | 'danger' | 'info' {
  ...
  if (['void', 'voided', 'refunded', 'rejected', 'failed'].includes(status)) return 'danger'
  return 'info'
}
...
function printReceipt(payment: Payment) {
  ...
  receiptWindow.document.write(`<html><head><title>${receipt.receiptNumber}</title></head><body style="font-family:Arial,sans-serif;padding:32px;color:#172033"><h1>Plamenco Dental Co.</h1><p>Payment Receipt</p><hr/><p><strong>Receipt:</strong> ${receipt.receiptNumber}</p><p><strong>Payment:</strong> ${payment.paymentNumber}</p><p><strong>Patient:</strong> ${patientName(receipt.patientId)}</p><p><strong>Invoice:</strong> ${invoice?.invoiceNumber ?? payment.invoiceId}</p><p><strong>Method:</strong> ${getPaymentMethodLabel(payment.paymentMethod)}</p><h2>${formatCurrency(receipt.amountCents)}</h2><p><strong>Remaining balance:</strong> ${formatCurrency(receipt.remainingBalanceCents)}</p><p style="margin-top:40px;color:#667085">This is a payment acknowledgement, not a BIR Official Receipt.</p></body></html>`)
