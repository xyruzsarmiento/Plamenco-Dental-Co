      ...
      {showAssign && (
        <div className="forms-v28-modal-backdrop" role="presentation" onMouseDown={(event) => event.target === event.currentTarget && !busy && setShowAssign(false)}>
          <section className="forms-v28-modal" role="dialog" aria-modal="true" aria-labelledby="forms-v28-assign-title">
            ...
            <div className="forms-v28-modal-body">
              <section className="forms-v28-form-section"><div className="forms-v28-section-title"><span>01</span><div><h3>Recipient</h3><p>Select the patient and published version.</p></div></div><PatientSearchCombobox patients={patients} value={assignForm.patientId} required placeholder="Search by name, patient ID, phone or email" onSelect={(patient) => setAssignForm({ ...assignForm, patientId: patient?.patientId ?? '' })} /><label>Published version<select value={assignForm.versionId} onChange={(event) => setAssignForm({ ...assignForm, versionId: event.target.value })}><option value="">Select published version</option>{publishedVersionOptions.map((version) => <option key={version.id} value={version.id}>{selectedTemplate?.title ?? 'Selected form'} · v{version.versionNumber}</option>)}</select></label></section>
