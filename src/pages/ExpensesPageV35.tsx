...
import { ExpenseTrendV66 } from '../components/ui/ExpenseTrendV66'
...
export function ExpensesPageV35() {
  ...
  return <section className="expenses35-shell">
    ...
    <div className="analytics35-grid expenses35-analytics expenses56-analytics">
      <section className="analytics35-card analytics35-wide expenses35-trend-card expenses66-trend-card"><header><span>Cost trend</span><h3>Operating cost trend</h3><p>Recorded operating expenses across the current year. Hover or keyboard-focus any period to inspect its exact recorded amount.</p></header><ExpenseTrendV66 rows={snapshot.trend.map((row) => ({ label: row.label, value: row.expensesCents }))} formatter={formatReportCurrency} /></section>
