  ...
  return <div className="ppx-shell">
    ...
    <main className="ppx-main">
      ...
      <div className="ppx-content">
        ...
        {tab === 'documents' && <section className="ppx-page"><SectionHeading eyebrow="SECURE FILES" title="Documents" copy="Patient-visible documents download using short-lived secure links."/><div className="ppx-document-grid">{documents.map((document)=><article key={document.id}><span><FileUser size={22}/></span><div><strong>{document.fileName}</strong><p>{document.category.replaceAll('_',' ')} · {clinicDate(document.uploadDate)}</p><small>{document.description || 'Shared by your clinic'}</small></div><a href={document.content || '#'} data-patient-document-id={document.id} download={document.fileName}>Download</a></article>)}{!documents.length&&<EmptyState icon={FileUser} title="No shared documents" copy="X-rays, consent files, referrals and other patient-visible documents will appear here."/>}</div></section>}
