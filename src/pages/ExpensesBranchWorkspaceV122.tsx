...
const tabs: Array<{key:Tab;label:string;helper:string}> = [
  {key:'ledger',label:'Expense Ledger',helper:'Branch operating costs'},
  {key:'due',label:'Due & Payables',helper:'Bills needing attention'},
  {key:'scheduled',label:'Scheduled Expenses',helper:'Branch recurring costs'},
  {key:'small_cash',label:'Small Cash Purchases',helper:'Branch petty cash'},
...
function initials(expense:Expense){return(expense.payeeName||expense.description||'EX').split(' ').filter(Boolean).slice(0,2).map((w)=>w[0]?.toUpperCase()).join('')}

function CostTrend({labels,values}:{labels:string[];values:number[]}){
  const width=920,height=300,left=44,right=20,top=24,bottom=40
...
function ScopedBranchExpenses(){
  ...
  const allExpenses=useMemo(()=>{void refreshKey;return getExpenses()},[refreshKey])
  ...
  const payments=useMemo(()=>{void refreshKey;return getExpensePayments().filter((p)=>expenseIds.has(p.expenseId))},[expenseIds,refreshKey])
  ...
  const overview=useMemo(()=>{void refreshKey;return activeBranchId?getExpenseOverview(activeBranchId):getExpenseOverview(undefined)},[activeBranchId,refreshKey])
  const snapshot=useMemo(()=>buildEnterpriseReportSnapshot({filters:{preset:'this_year',branchId:activeBranchId??undefined,authorizedBranchIds}}),[activeBranchId,authorizedBranchIds,refreshKey])
  ...
  const paidThisMonth=payments.filter((p)=>p.paymentDate.startsWith(currentMonth)).reduce((sum,p)=>sum+p.amountCents,0)
  ...
  function createSupplierExpense(receiptId:string){try{const created=createExpenseFromPurchaseReceipt(receiptId);refresh(`${created.expenseNumber} created from this ${activeBranch.name} purchase receipt.`)}catch(cause){setError(cause instanceof Error?cause.message:'Unable to create supplier expense.')}}

  return <section className="ex57-page ex122-page" data-expense-scope-key={`expenses:${activeBranchId}:${authorizedBranchIds.join(',')}`}>
    <section className="ex57-hero"><div><span>FINANCE OPERATIONS · {activeBranch.name.toUpperCase()}</span><h1>Expense Control Center</h1><p>Branch operating costs, payables, recurring expenses, small cash and supplier bills for {activeBranch.name}.</p></div><div className="ex57-actions">{permissions.can('expenses.create')&&<Button onClick={()=>setDialog('add_expense')}><Plus size={16}/>Add expense</Button>}{permissions.can('expenses.record_payment')&&<Button variant="secondary" onClick={()=>setDialog('petty_cash')}><Banknote size={16}/>Small cash purchase</Button>}{permissions.can('expenses.create')&&<Button variant="secondary" onClick={()=>setDialog('add_vendor')}><Store size={16}/>Add vendor</Button>}</div></section>
    <section className="ex122-branch-context"><div><span>ACTIVE COST CENTER</span><strong>{activeBranch.name}</strong><small>Branch-owned entries created here are locked to this workspace.</small></div><Badge tone="info">Branch expense scope</Badge></section>
    ...
    <section className="ex57-kpis"><article><i><ReceiptText size={17}/></i><span>This month</span><strong>{formatExpenseCurrency(overview.thisMonthCents)}</strong><small>{activeBranch.name} non-void expenses</small></article><article><i><CircleDollarSign size={17}/></i><span>Open payables</span><strong>{formatExpenseCurrency(overview.unpaidCents)}</strong><small>Outstanding {activeBranch.name} balances</small></article><article><i><CalendarClock size={17}/></i><span>Needs attention</span><strong>{overview.dueSoon+overview.overdue}</strong><small>{overview.overdue} overdue · {overview.dueSoon} due soon</small></article><article><i><WalletCards size={17}/></i><span>Paid this month</span><strong>{formatExpenseCurrency(paidThisMonth)}</strong><small>Payments tied to this branch</small></article></section>
    <section className="ex57-trend-card"><header><div><span>COST TREND · {activeBranch.name.toUpperCase()}</span><h2>Operating cost trend</h2><p>Only expenses owned by this branch are counted. Clinic-wide costs are intentionally excluded.</p></div><div className="ex57-trend-total"><TrendingUp size={17}/><span>Year view</span><strong>{formatExpenseCurrency(snapshot.trend.reduce((sum,row)=>sum+row.expensesCents,0))}</strong></div></header><CostTrend labels={snapshot.trend.map((r)=>r.label)} values={snapshot.trend.map((r)=>r.expensesCents)}/></section>
    <section className="ex57-command"><div className="ex57-tabs" role="tablist">{tabs.map((item)=><button key={item.key} className={tab===item.key?'is-active':''} onClick={()=>setTab(item.key)}><strong>{item.label}</strong><span>{item.helper}</span></button>)}</div><div className="ex57-filters"><label className="ex57-search"><Search size={16}/><input value={search} onChange={(e)=>setSearch(e.target.value)} placeholder="Search expense, payee or reference"/></label><span className="ex122-filter-branch">{activeBranch.name}</span><select value={category} onChange={(e)=>setCategory(e.target.value)}><option value="all">All categories</option>{getExpenseCategories().map((c)=><option key={c.id} value={c.id}>{c.name}</option>)}</select><select value={status} onChange={(e)=>setStatus(e.target.value as typeof status)}>{['all','draft','unpaid','partially_paid','paid','void','cancelled'].map((s)=><option key={s} value={s}>{s.replaceAll('_',' ')}</option>)}</select><select value={source} onChange={(e)=>setSource(e.target.value as typeof source)}>{['all','manual','purchase_receipt','purchase_order','recurring','other'].map((s)=><option key={s} value={s}>{s.replaceAll('_',' ')}</option>)}</select></div></section>
    {error&&<div className="ex57-alert is-error">{error}</div>}{message&&<div className="ex57-alert is-success">{message}</div>}
    {tab==='ledger'&&<section className="ex57-section"><header><div><span>EXPENSE LEDGER · {activeBranch.name.toUpperCase()}</span><h3>{filtered.length} records</h3><p>Company-wide expenses are not included in this branch ledger.</p></div></header><div className="ex57-card-grid">{pageItems(filtered,page).map((e)=><article className="ex57-mini" key={e.id}><div><StatusBadge status={e.status} variant="compact"/><span>{e.expenseNumber}</span></div><h4>{e.description}</h4><p>{e.payeeName} · {categoryName(e.categoryId)}</p><footer><span>{formatDate(e.expenseDate)}</span><strong>{formatExpenseCurrency(e.balanceCents)}</strong></footer></article>)}{!filtered.length&&<div className="ex57-empty is-wide"><ReceiptText size={28}/><h3>No branch expenses in this view</h3></div>}</div></section>}
    {tab==='due'&&<section className="ex57-section"><header><div><span>PAYABLES WATCHLIST</span><h3>{activeBranch.name} due & payables</h3><p>Only branch-owned outstanding expenses appear here.</p></div><b>{due.length}</b></header><div className="ex57-card-grid">{pageItems(due,page).map((e)=><article className="ex57-mini" key={e.id}><div><StatusBadge status={getExpenseDueStatus(e)} variant="compact"/><span>{e.expenseNumber}</span></div><h4>{e.description}</h4><p>{e.payeeName}</p><footer><span>Due {formatDate(e.dueDate)}</span><strong>{formatExpenseCurrency(e.balanceCents)}</strong></footer></article>)}{!due.length&&<div className="ex57-empty is-wide"><CalendarClock size={28}/><h3>No urgent branch payables</h3></div>}</div></section>}
    {tab==='scheduled'&&<section className="ex57-section"><header><div><span>SCHEDULED EXPENSES</span><h3>{activeBranch.name} recurring costs</h3><p>Rent, utilities and other predictable branch obligations remain tied to this branch.</p></div>{permissions.can('expenses.manage_recurring')&&<Button size="sm" onClick={()=>setDialog('recurring')}><Plus size={14}/>New schedule</Button>}</header><div className="ex57-card-grid">{pageItems(recurring,page).map((r)=><article className="ex57-mini" key={r.id}><div><StatusBadge status={r.status} variant="compact"/><span>{r.frequency}</span></div><h4>{r.name}</h4><p>{r.payeeName} · {activeBranch.name}</p><footer><span>Next due {formatDate(r.nextDueDate)}</span><strong>{r.defaultAmountCents?formatExpenseCurrency(r.defaultAmountCents):'Variable amount'}</strong></footer></article>)}{!recurring.length&&<div className="ex57-empty is-wide"><CalendarClock size={28}/><h3>No branch schedules</h3></div>}</div></section>}
    {tab==='small_cash'&&<section className="ex57-section"><header><div><span>SMALL CASH PURCHASES</span><h3>{activeBranch.name} petty cash</h3><p>Immediate minor purchases are recorded only against this branch.</p></div>{permissions.can('expenses.record_payment')&&<Button size="sm" onClick={()=>setDialog('petty_cash')}><Banknote size={14}/>Record small cash purchase</Button>}</header><div className="ex57-card-grid">{pageItems(petty,page).map((e)=><article className="ex57-mini" key={e.id}><div><StatusBadge status={e.status} variant="compact"/><span>{e.expenseNumber}</span></div><h4>{e.description}</h4><p>{e.payeeName}</p><footer><span>{formatDate(e.expenseDate)}</span><strong>{formatExpenseCurrency(e.totalCents)}</strong></footer></article>)}{!petty.length&&<div className="ex57-empty is-wide"><Banknote size={28}/><h3>No branch small cash purchases</h3></div>}</div></section>}
    {tab==='supplier_bills'&&<section className="ex57-section"><header><div><span>PROCUREMENT HANDOFF</span><h3>{activeBranch.name} supplier bills</h3><p>Only purchase receipts delivered to this branch are shown.</p></div><b>{receipts.length}</b></header><div className="ex57-card-grid">{pageItems(receipts,page).map((r)=>{const linked=expenses.find((e)=>e.sourceType==='purchase_receipt'&&e.sourceId===r.id);return <article className="ex57-mini" key={r.id}><div>{linked?<StatusBadge status={linked.status} label={linked.expenseNumber} variant="compact"/>:<Badge tone="info">Not linked</Badge>}<span>{r.receiptNumber}</span></div><h4>{activeBranch.name}</h4><p>Received {formatDate(r.receivedDate)}</p><footer><strong>{formatExpenseCurrency(r.totalCostCents)}</strong>{!linked&&permissions.can('expenses.create')&&<Button size="sm" onClick={()=>createSupplierExpense(r.id)}>Create expense</Button>}</footer></article>})}{!receipts.length&&<div className="ex57-empty is-wide"><FileText size={28}/><h3>No branch supplier receipts</h3></div>}</div></section>}
    ...
    {dialog&&<BranchScopedExpenseActionModal type={dialog} branch={activeBranch} onClose={()=>setDialog(null)} onSuccess={()=>{setDialog(null);refresh('Expense workspace updated.')}}/>}
  </section>
}

export function ExpensesBranchWorkspaceV122(){
  const {user}=useAuth()
