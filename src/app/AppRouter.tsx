...
function BookRoute() {
  ...
  if (isAuthenticated && user?.role === 'patient' && user.patientId) return <Navigate to={`/portal/${user.patientId}?tab=booking`} replace />
...
function RouteRobotsMeta() {
  ...
  useEffect(() => {
    ...
    const privateRoutePrefixes = ['/app', '/portal', '/staff', '/dentist', '/super-admin', '/login', '/register', '/forgot-password', '/reset-password', '/accept-invite']
