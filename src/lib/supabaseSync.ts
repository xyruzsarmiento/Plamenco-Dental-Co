...
const tablesToSync: SyncTableEntry[] = [
  ...
  },
  {
    ...
    map: (row: any) => ({
      ...
      clinicalFindings: row.clinical_findings ?? row.findings ?? '',
      assessment: row.assessment ?? row.diagnosis ?? '',
      treatmentPerformed: row.treatment_performed ?? row.treatment_notes ?? '',
      recommendations: row.recommendations ?? row.treatment_plan ?? '',
