...
function readStoredScope(userId: string): BranchScope | null {
  try {
    ...
    if (parsed.kind === 'branch' && typeof parsed.branchId === 'string' && parsed.branchId) return { kind: 'branch', branchId: parsed.branchId }
  } catch {
    // UX persistence is never authorization.
  }
  return null
}

function persistScope(userId: string, scope: BranchScope) {
  try { window.localStorage.setItem(storageKey(userId), JSON.stringify(scope)) } catch { /* optional UX persistence */ }
}
...
async function loadStaffAssignments(profileId: string): Promise<AssignmentRow[]> {
  if (!supabase) return []
  const { data, error } = await supabase.from('staff_branch_assignments').select('branch_id, is_primary, status').eq('profile_id', profileId).eq('status', 'active')
...
async function loadProviderAssignments(profileId: string): Promise<AssignmentRow[]> {
  if (!supabase) return []
  const { data: provider, error: providerError } = await supabase.from('providers').select('id').eq('profile_id', profileId).eq('status', 'active').maybeSingle()
  if (providerError) throw new Error(`Unable to resolve dentist profile: ${providerError.message}`)
  if (!provider?.id) return []
  const { data, error } = await supabase.from('provider_branch_assignments').select('branch_id, is_primary, status').eq('provider_id', provider.id).eq('status', 'active')
...
export function BranchProvider({ children }: { children: ReactNode }) {
  ...
  const value = useMemo<BranchContextValue>(() => ({
    availableBranches, activeBranchId, activeBranch, authorizedBranchIds, canViewAllBranches,
    isAllBranchesMode: scope?.kind === 'all', isLoading, error, hasBranchAccess: authorizedBranchIds.length > 0,
    setActiveBranch, setAllBranches, refreshBranchAccess,
...
export function useOptionalBranchContext() { return useContext(BranchContext) }
