...
export type StaffBranchAssignmentAdminRow = {
  id: string
  profileId: string
  branchId: string
  isPrimary: boolean
...
export type ProviderBranchAssignmentAdminRow = {
  id: string
  providerId: string
  branchId: string
  isPrimary: boolean
...
export async function loadStaffBranchAssignmentsAdmin(): Promise<StaffBranchAssignmentAdminRow[]> {
  ...
  const { data, error } = await db
    ...
    .select('id, profile_id, branch_id, is_primary, status')
    .order('is_primary', { ascending: false })
  if (error) throw new Error(`Unable to load staff branch assignments: ${error.message}`)
  return (data ?? []).map((row) => ({
    id: String(row.id), profileId: String(row.profile_id), branchId: String(row.branch_id),
    isPrimary: Boolean(row.is_primary), status: row.status as 'active' | 'inactive',
  }))
}
...
export async function loadProviderBranchAssignmentsAdmin(): Promise<ProviderBranchAssignmentAdminRow[]> {
  ...
  const { data, error } = await db
    ...
    .select('id, provider_id, branch_id, is_primary, status')
    .order('is_primary', { ascending: false })
  if (error) throw new Error(`Unable to load dentist branch assignments: ${error.message}`)
  return (data ?? []).map((row) => ({
    id: String(row.id), providerId: String(row.provider_id), branchId: String(row.branch_id),
    isPrimary: Boolean(row.is_primary), status: row.status as 'active' | 'inactive',
  }))
}
...
export async function replaceStaffBranchAssignmentsPersisted(profileId: string, branchIds: string[], primaryBranchId?: string) {
  ...
  const { data, error } = await db.rpc('replace_staff_branch_assignments', {
    p_profile_id: profileId,
    p_branch_ids: uniqueIds,
    p_primary_branch_id: primary,
  })
...
export async function replaceProviderBranchAssignmentsPersisted(providerId: string, branchIds: string[], primaryBranchId?: string) {
  ...
  const uniqueIds = [...new Set(branchIds.filter(Boolean))]
  const primary = primaryBranchId && uniqueIds.includes(primaryBranchId) ? primaryBranchId : uniqueIds[0] ?? null
  const { data, error } = await db.rpc('replace_provider_branch_assignments', {
