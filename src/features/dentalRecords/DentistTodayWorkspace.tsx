...
import { acceptUnassignedAppointmentPersisted } from '../appointments/appointmentPersistence'
...
export function DentistTodayWorkspace() {
  ...
  useEffect(() => {
    ...
    void loadPatientsFromSupabase()
      ...
      .catch((error) => {
        if (import.meta.env.DEV) console.warn('[dentist workspace] patient refresh failed', error)
      })
    return () => { active = false }
  }, [user?.id])
  ...
  const availableRequests = useMemo(() => appointments.filter((appointment) => {
    ...
    if (appointment.status !== 'pending' || appointment.providerId) return false
    if (appointment.proposedProviderId) return false
    if (!appointment.branchId || !providerBranchIds.has(appointment.branchId)) return false
    if (isAllBranchesMode && !authorizedBranchIds.includes(appointment.branchId)) return false
    if (!isAllBranchesMode && appointment.branchId !== activeBranchId) return false
    return true
  ...
  async function refresh() {
    ...
    const rows = await loadAppointmentsForBranchScope({ branchId: activeBranchId, isAllBranchesMode, userId: user.id, bypassCache: true })
    setAppointments(rows)
  }

  async function acceptRequest(appointment: Appointment) {
    if (!user || actionPendingId) return
    setActionPendingId(`accept:${appointment.id}`)
    setMessage(null)
    try {
      const updated = await acceptUnassignedAppointmentPersisted({ appointmentId: appointment.id, providerId: provider?.id, actor: user.name || user.email, expectedUpdatedAt: appointment.updatedAt })
      await refresh()
      setMessage(`Appointment ${updated.appointmentNumber ?? updated.id} accepted and confirmed.`)
    } catch (cause) {
      setMessage(cause instanceof Error ? cause.message : 'Appointment request could not be updated.')
      void refresh()
    } finally {
  ...
  return <section className="dentist129" data-provider-id={provider.id} data-dentist-branch={activeBranchId ?? 'all'}>
    <header className="dentist129-hero">
      <div className="dentist129-hero-copy">
        ...
        <p>Your assigned visits and open appointment requests are resolved from your authenticated provider profile and selected branch access.</p>
    ...
    <section className="dentist129-card dentist129-nominations" aria-label="Available appointment requests">
      <div className="dentist129-card-head"><div><span className="dentist129-kicker">Appointment requests</span><h3>Open requests</h3><p>These pending unassigned visits match your active branch access. Accepting confirms the appointment and assigns it to you.</p></div><StatusBadge status="pending" label={`${availableRequests.length} pending`} variant="compact" /></div>
      <div className="dentist129-schedule">{availableRequests.map((appointment) => {
        ...
        const busyAccept = actionPendingId === `accept:${appointment.id}`
        return <article key={appointment.id} className="dentist129-schedule-row dentist129-request-row"><div className="dentist129-time"><strong>{formatTime(appointment.startTime)}</strong><small>{appointment.date}</small></div><div className="dentist129-patient"><strong>{patientDisplay(patient)}</strong><small>{service?.name ?? appointment.reasonForVisit ?? 'Dental visit'} - {service?.duration ?? appointment.durationMinutes ?? 30} min</small><small>{branch?.name ?? 'Clinic branch'}{appointment.notes ? ` - ${appointment.notes}` : ''}</small></div><div className="dentist129-meta"><StatusBadge status="pending" label="Unassigned" variant="compact" /><Button size="sm" disabled={Boolean(actionPendingId)} onClick={() => void acceptRequest(appointment)}><CheckCircle2 size={14}/>{busyAccept ? 'Accepting...' : 'Accept & assign to me'}</Button></div></article>
      })}{!availableRequests.length && <div className="dentist129-empty">No appointment requests are currently available in {scopeName}.</div>}</div>
    </section>
    ...
    <div className="dentist129-grid">
      <section className="dentist129-card">
        ...
        <div className="dentist129-schedule">{todayAppointments.map((appointment) => {
          const patient = patientMap.get(appointment.patientId)
          return <article key={appointment.id} className="dentist129-schedule-row"><div className="dentist129-time">{formatTime(appointment.startTime)}</div><div className="dentist129-patient"><strong>{patientDisplay(patient)}</strong><small>{serviceMap.get(appointment.serviceId)?.name ?? appointment.reasonForVisit ?? 'Visit'}</small></div><div className="dentist129-meta"><span className="dentist129-branch">{appointment.branchId ? branchMap.get(appointment.branchId)?.name ?? 'Branch' : 'Branch not recorded'}</span><StatusBadge status={appointment.status} label={statusLabel(appointment.status)} variant="compact" />{['checked_in','waiting'].includes(appointment.status) && permissions.can('appointments.start') && <button className="text-button" type="button" onClick={() => startVisit(appointment)}>Start</button>}{patient ? <Link className="text-button" to={`/app/patients/${encodeURIComponent(patient.patientId)}`}>Patient</Link> : <button className="text-button" type="button" disabled title="Patient record could not be found.">Patient unavailable</button>}</div></article>
      ...
      <aside className="dentist129-card">
        ...
        {nextAppointment ? (() => { const patient = patientMap.get(nextAppointment.patientId); const service = serviceMap.get(nextAppointment.serviceId); const alerts = [patient?.allergies, patient?.medicalConditions, patient?.currentMedications].filter(Boolean).length; return <div className="dentist129-next"><div className="dentist129-next-main"><strong>{patientDisplay(patient)}</strong><small>{service?.name ?? nextAppointment.reasonForVisit ?? 'Visit'}</small></div><div className="dentist129-next-grid"><div><span>Status</span><strong>{statusLabel(nextAppointment.status)}</strong></div><div><span>Medical context</span><strong>{alerts ? `${alerts} item${alerts === 1 ? '' : 's'} to review` : 'No alerts recorded'}</strong></div></div><div className="dentist129-action-row">{patient ? <Link to={`/app/patients/${encodeURIComponent(patient.patientId)}`}><Button size="sm" variant="secondary">Open Patient</Button></Link> : <Button size="sm" variant="secondary" disabled>Patient record unavailable</Button>}{getClinicalVisitByAppointment(nextAppointment.id) && <Link to={`/app/dental-records?appointment=${encodeURIComponent(nextAppointment.id)}`}><Button size="sm">Clinical Record</Button></Link>}</div></div> })() : <div className="dentist129-empty">No remaining patient is scheduled today.</div>}
      </aside>
    </div>

    <section className="dentist129-card"><div className="dentist129-card-head"><div><span className="dentist129-kicker">Recent patients</span><h3>Recently completed visits</h3><p>Derived from your actual completed appointments in the current authorized scope.</p></div><UsersRound size={20}/></div><div className="dentist129-recent">{recentPatients.map((appointment) => { const patient = patientMap.get(appointment.patientId); return patient ? <Link key={appointment.id} to={`/app/patients/${encodeURIComponent(patient.patientId)}`}><strong>{patientDisplay(patient)}</strong><small>{appointment.date} - {serviceMap.get(appointment.serviceId)?.name ?? 'Completed visit'}</small></Link> : <span key={appointment.id} className="dentist129-missing-patient"><strong>Patient record unavailable</strong><small>{appointment.date} - {serviceMap.get(appointment.serviceId)?.name ?? 'Completed visit'}</small></span> })}{!recentPatients.length && <div className="dentist129-empty">No recently completed visits in this scope.</div>}</div></section>
  </section>
}
