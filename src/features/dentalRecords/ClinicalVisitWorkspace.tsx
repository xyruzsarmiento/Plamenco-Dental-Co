...
export function ClinicalVisitWorkspace({ record, patient, appointment, branch, provider, services, actor, canEditDraft, canFinalize, canAmend, canCreateTreatment, canCreatePrescription, canUploadDocuments, onClose, onRecordChange }: ClinicalVisitWorkspaceProps) {
  ...
  function addPrescription() {
    createPrescription({ patientId: patient.patientId, dentalRecordId: record.id, appointmentId: appointment?.id, branchId: branch?.id, providerId: provider?.id, providerNameSnapshot: provider?.displayName, items: prescriptionItems, notes: prescriptionNotes, prescribedBy: provider?.displayName ?? actor, prescriptionDate: record.recordDate })
  ...
  return <div className="modal-backdrop" role="presentation"><section className="modal clinical-workspace-modal" role="dialog" aria-modal="true" aria-labelledby="clinical-workspace-title">
    ...
    <div className="clinical-workspace-grid"><main className="clinical-workspace-main">
      ...
      <section className="clinical-section"><div className="clinical-section-header"><h3>Clinical Documentation</h3><Badge tone={isFinal ? 'success':'warning'}>{record.status}</Badge></div>
      ...
      <section className="clinical-section"><div className="clinical-section-header"><h3>Prescriptions</h3><Pill size={16}/></div>{canCreatePrescription&&!isFinal&&<div className="clinical-inline-form">{prescriptionItems.map((item,index)=><div className="clinical-medication-grid" key={index}><Input label="Medication" value={item.medication} onChange={(e)=>setPrescriptionItems(c=>c.map((x,i)=>i===index?{...x,medication:e.target.value}:x))}/><Input label="Strength" value={item.strength} onChange={(e)=>setPrescriptionItems(c=>c.map((x,i)=>i===index?{...x,strength:e.target.value}:x))}/><Input label="Dosage" value={item.dosage} onChange={(e)=>setPrescriptionItems(c=>c.map((x,i)=>i===index?{...x,dosage:e.target.value}:x))}/><Input label="Frequency" value={item.frequency} onChange={(e)=>setPrescriptionItems(c=>c.map((x,i)=>i===index?{...x,frequency:e.target.value}:x))}/><Input label="Duration" value={item.duration} onChange={(e)=>setPrescriptionItems(c=>c.map((x,i)=>i===index?{...x,duration:e.target.value}:x))}/><Input label="Instructions" value={item.instructions} onChange={(e)=>setPrescriptionItems(c=>c.map((x,i)=>i===index?{...x,instructions:e.target.value}:x))}/></div>)}<Button variant="secondary" onClick={()=>setPrescriptionItems(c=>[...c,emptyPrescriptionItem])}>Add Medication</Button><Textarea label="Prescription notes" value={prescriptionNotes} onChange={(e)=>setPrescriptionNotes(e.target.value)}/><Button onClick={addPrescription}>Save Prescription</Button></div>}<div className="clinical-linked-list">{prescriptions.map(p=><article key={p.id}><strong>{p.medication}</strong><span>{p.providerNameSnapshot||p.prescribedBy} - {formatDate(p.prescriptionDate)}</span></article>)}{!prescriptions.length&&<div className="empty-state-panel">No prescriptions recorded for this visit.</div>}</div></section>
      <section className="clinical-section"><div className="clinical-section-header"><h3>Documents</h3><FileText size={16}/></div>{canUploadDocuments&&<DocumentUploadPanel patientId={patient.patientId} onUpload={(payload)=>{createDocument({...payload,clinicalVisitId:record.id,uploadedBy:actor});refreshRelated();setMessage('Document attached.')}}/>}<div className="clinical-linked-list">{documents.map(d=><article key={d.id}><strong>{d.fileName}</strong><span>{d.category} - {d.uploadedBy}</span></article>)}{!documents.length&&<div className="empty-state-panel">No documents attached to this visit.</div>}</div></section>
