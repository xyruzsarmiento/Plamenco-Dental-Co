...
export function InvoiceCreatorButtonV32({ onSuccess }: { onSuccess?: (invoiceId: string) => void }) {
  ...
  async function submit() {
    ...
    try {
      setSubmitting(true)
      const invoice = await createInvoicePersisted({
          ...
          <section className="inv32-modal" role="dialog" aria-modal="true" aria-labelledby="inv32-title">
            ...
            <div className="inv32-body">
              <main className="inv32-form-column">
                ...
                <section className="inv32-section">
                  <div className="inv32-section-head"><span>2</span><div><h3>Unbilled clinical charges</h3><p>Optional. Select existing unbilled charges for this patient; selected charges are marked invoiced atomically with invoice creation.</p></div></div>
                  {!patientId ? <div className="inv32-soft-empty">Select a patient to see eligible unbilled charges.</div> : eligibleCharges.length ? (
                    <div className="inv32-charge-list">{eligibleCharges.map((charge) => <label key={charge.id} className={selectedCharges.includes(charge.id) ? 'is-selected' : ''}><input type="checkbox" checked={selectedCharges.includes(charge.id)} onChange={() => toggleCharge(charge.id)} disabled={submitting} /><div><strong>{charge.description}</strong><span>{charge.providerNameSnapshot || 'Provider not recorded'} · Qty {charge.quantity}</span></div><b>{formatCurrency(charge.finalAmountCents)}</b></label>)}</div>
                  ) : <div className="inv32-soft-empty">No unbilled charges are available for this patient.</div>}
                </section>

                <section className="inv32-section">
                  <div className="inv32-section-head inv32-section-head-action"><div className="inv32-section-head-copy"><span>3</span><div><h3>Manual invoice lines</h3><p>Add services or adjustments that are not represented by an existing charge.</p></div></div><Button variant="secondary" size="sm" icon={<Plus size={14} />} onClick={() => setLines((current) => [...current, emptyLine()])} disabled={submitting}>Add line</Button></div>
                  <div className="inv32-lines">{lines.map((line, index) => <div className="inv32-line" key={line.key}><div className="inv32-line-index">{index + 1}</div><label className="inv32-line-description"><span>Description</span><input value={line.description} onChange={(event) => updateLine(line.key, { description: event.target.value })} placeholder="e.g. Professional cleaning" disabled={submitting}/></label><label><span>Qty</span><input type="number" min="1" step="1" value={line.quantity} onChange={(event) => updateLine(line.key, { quantity: event.target.value })} disabled={submitting}/></label><label><span>Price (PHP)</span><input type="number" min="0" step="0.01" value={line.unitPrice} onChange={(event) => updateLine(line.key, { unitPrice: event.target.value })} placeholder="0.00" disabled={submitting}/></label><button type="button" className="inv32-remove" onClick={() => setLines((current) => current.length === 1 ? [emptyLine()] : current.filter((entry) => entry.key !== line.key))} aria-label={`Remove line ${index + 1}`} disabled={submitting}><Trash2 size={16} /></button></div>)}</div>
                </section>
                ...
                <section className="inv32-section">
                  ...
                  <textarea rows={4} value={notes} onChange={(event) => setNotes(event.target.value)} placeholder="Add billing notes or context..." disabled={submitting} />
                </section>
              ...
              <aside className="inv32-summary-column">
                ...
                <div className="inv32-trust-card"><FilePlus2 size={18} /><div><strong>Billing source of truth</strong><p>PostgreSQL validates the invoice and returns the confirmed record before this dialog reports success.</p></div></div>
              </aside>
            </div>
