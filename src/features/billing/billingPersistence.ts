...
function replaceInvoice(invoice: Invoice) {
  saveStoredInvoices([invoice, ...getStoredInvoices().filter((entry) => entry.id !== invoice.id)])
}

function replacePayment(payment: Payment) {
  saveStoredPayments([payment, ...getStoredPayments().filter((entry) => entry.id !== payment.id)])
}
...
export async function createInvoicePersisted(input: {
  patientDbId: string
  branchId?: string
  invoiceDate: string
  dueDate?: string
  ...
  const { data, error } = await db.rpc('create_invoice_from_items', {
    p_patient_id: input.patientDbId,
  ...
  const invoice = mapInvoiceRow(data.invoice as Record<string, any>)
  replaceInvoice(invoice)
  return invoice
}

export async function recordManualPaymentPersisted(input: {
  invoiceId: string
  amountCents: number
  ...
  notes?: string
  clientRequestId?: string
}): Promise<{ payment: Payment; invoice: Invoice; receipt: Receipt }> {
  const db = requireDatabase()
  const clientRequestId = input.clientRequestId ?? createUuid()
  ...
  if (error || !data?.payment || !data?.invoice || !data?.receipt) {
    throw rpcError('Payment was not recorded. No balance change was committed.', error)
  }
  const payment = mapPaymentRow(data.payment as Record<string, any>)
  const invoice = mapInvoiceRow(data.invoice as Record<string, any>)
  const receipt = mapReceiptRow(data.receipt as Record<string, any>)
...
export async function applyInvoiceDiscountPersisted(input: {
  invoiceId: string
  itemId: string
  type: DiscountType
  valueCents?: number
  ...
  const db = requireDatabase()
  const { data, error } = await db.rpc('apply_invoice_discount', {
    p_invoice_id: input.invoiceId,
  ...
  if (error || !data) throw rpcError('Discount was not applied. The invoice was left unchanged.', error)
...
export async function voidInvoicePersisted(invoiceId: string, reason: string): Promise<Invoice> {
  const db = requireDatabase()
  const { data, error } = await db.rpc('void_invoice', { p_invoice_id: invoiceId, p_reason: reason })
  if (error || !data) throw rpcError('The invoice could not be voided. It remains unchanged.', error)
  const invoice = mapInvoiceRow(data as Record<string, any>)
  replaceInvoice(invoice)
  return invoice
}

export async function refundPaymentPersisted(input: {
  paymentId: string
  amountCents: number
  reason: string
  ...
  if (error || !data?.refund || !data?.payment || !data?.invoice) {
    throw rpcError('The refund could not be recorded. No financial changes were committed.', error)
  }
  const refund = mapRefundRow(data.refund as Record<string, any>)
  const payment = mapPaymentRow(data.payment as Record<string, any>)
  const invoice = mapInvoiceRow(data.invoice as Record<string, any>)
  replaceRefund(refund)
  replacePayment(payment)
  replaceInvoice(invoice)
